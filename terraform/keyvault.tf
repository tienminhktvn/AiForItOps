...
# hoặc service principal của pipeline) — bắt buộc phải có, nếu không các
# azurerm_key_vault_secret trong keyvault-secrets.tf sẽ apply lỗi 403.
...
# Tùy chọn: cấp thêm quyền cho 1 user cụ thể khác (vd. đồng đội), nếu principal_id
# khác với người/SP đang chạy terraform — bicep: secretsOfficerRoleAssignment
